...
export class CommentResource {
  constructor(commentDbObject) {
    ...
    this.topReactionTypes = commentDbObject.topReactionTypes
      ? commentDbObject.topReactionTypes
      : [];
    if (commentDbObject.reactions && commentDbObject.reactions.length) {
      const reactionTypes = commentDbObject.reactions.map((reaction) => {
      ...
      reactionTypes.filter((reactionType) => reactionType !== null);
      if (reactionTypes.length) {
        const reactionCounts = reactionTypes.reduce((counts, { reactionType }) => {
          counts[reactionType] = (counts[reactionType] || 0) + 1;
          return counts;
        }, {});
        this.topReactionTypes = Object.keys(reactionCounts)
