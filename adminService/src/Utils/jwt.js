...
const tokenValidator = async ({ token, type, ip, userAgent, authTokenReference = "" }) => {
...
export const verifyJWT = async (data) => {
  ...
  try {
    decode = await tokenValidator({ token: authToken, type: "auth", ip, userAgent }).then((res) => {
     ...
     console.log(deviceToken, "deviceToken req.body admin")
     console.log(decode.payload.deviceToken, "deviceToken decode admin")
  
    if (decode.payload.userType !== type) {
      console.log(decode.payload.userType, type);
    ...
    console.log(error, "error part in admin service")
     if (error.message !== "JWT expired") {
      return {
        error: "admin Invalid authorization token. " + error.message,
      };
    }
